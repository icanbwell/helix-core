...
COPY Pipfile* /usr/src/helixcore/
...
ENV PROJECT_DIR=/usr/src/helixcore
ENV FLASK_APP=helixcore.api
...
COPY ./helixcore ${PROJECT_DIR}

# Copy the Pipfile.lock from the previous stage in case it was locked
COPY --from=python_packages /usr/src/helixcore/Pipfile.lock /usr/src/helixcore/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/helixcore/Pipfile.lock /tmp/Pipfile.lock
